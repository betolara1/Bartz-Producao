...
class App(tk.Tk):
    ...
    def _build_widgets(self):
        ...
        columns = ("lote", "status", "comentarios", "resp", "arquivo", "modificado", "tamanho")
        headers = {
            "lote": "Lote",
            "status": "Status",
        ...
        widths = {"lote": 85, "status": 105, "comentarios": 105, "resp": 150, "arquivo": 180, "modificado": 135, "tamanho": 75}
    ...
    def _scan_worker(self, root_path: str):
        try:
            ...
            self.result_queue.put(("ok", (entries, records)))
    ...
    def _on_scan_success(self, payload):
        ...
        for e in entries:
            ...
            for c in comms:
                ...
                if not self._is_first_scan and cid not in self.known_comment_ids:
        ...
        if new_comments_found:
            for entry_item, comment_item in new_comments_found[-3:]:
                ...
                title = f"Novo comentário — Lote {entry_item.lote}"
                msg = f"{autor}: \"{texto}\""
                ToastNotification(
                    self,
                    title=title,
    ...
    def _schedule_auto_refresh(self):
        interval_ms = max(5, int(self.cfg.get("refresh_seconds", 30))) * 1000
    ...
    def _render(self, entries: list[PdfEntry]):
        ...
        self.notebook.tab(0, text=f"Pendentes ({len(rows_pendentes)})")
        self.notebook.tab(1, text=f"Concluídos ({len(rows_concluidos)})")
